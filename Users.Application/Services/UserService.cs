...
public class UserService : IUserService
{
    ...
    public async Task<ResultDto<UserDto>> CreateUser(UserRequestDto dto, bool isAdmin = false)
    {
        ...
        if (userExists)
        {
            return ResultDto<UserDto>.Fail(ValueObjects.Error.BadRequest(ExceptionMessages.EmailAlreadyExistsException));
        }
    ...
    public async Task<ResultDto> DeleteUser(Guid id)
    {
        ...
        if (user is null)
        {
            return ResultDto.Fail(ValueObjects.Error.BadRequest(ExceptionMessages.UserNotExistsException));
        }
